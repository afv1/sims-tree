...
func (uq *UserQuery) QueryFamilies() *FamilyQuery {
	query := &FamilyQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFamilies: uq.withFamilies.Clone(),
...
func (uq *UserQuery) WithFamilies(opts ...func(*FamilyQuery)) *UserQuery {
	query := &FamilyQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withFamilies = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFamilies; query != nil {
		if err := uq.loadFamilies(ctx, query, nodes,
			func(n *User) { n.Edges.Families = []*Family{} },
			func(n *User, e *Family) { n.Edges.Families = append(n.Edges.Families, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadFamilies(ctx context.Context, query *FamilyQuery, nodes []*User, init func(*User), assign func(*User, *Family)) error {
	...
	query.Where(predicate.Family(func(s *sql.Selector) {
		s.Where(sql.InValues(user.FamiliesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_families
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_families" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_families" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
