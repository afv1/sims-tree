...
// The RelativeFunc type is an adapter to allow the use of ordinary
// function as Relative mutator.
type RelativeFunc func(context.Context, *ent.RelativeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RelativeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.RelativeMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RelativeMutation", m)
	}
	return f(ctx, mv)
}
